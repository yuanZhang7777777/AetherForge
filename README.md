...
4. 预备生成：legacy 链路保留 N1 可选视觉识别 + N2 分槽提示词；生产使用 `PROMPT_PIPELINE_MODE=gpt55_single`，由 APIMart 单节点一次完成商品理解、身份锁、`style_brief` 和固定 9 张图提示词。当前单节点实际调用 `backend/prompts.py::n_prepare_single_gpt55_system()`，提示词模型默认 `gpt-5-nano-2025-08-07`。参考商品图始终传给该节点；「AI 识别」仅控制是否把识别出的商品名称和事实回填到商品卡。
