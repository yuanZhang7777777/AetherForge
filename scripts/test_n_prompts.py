"""真实 DeepSeek 验证三节点「写提示词」节点：一次调用产出 9 张最终英文提示词 + style_brief。

断言：
1. prompts 长度 = 9，slot 覆盖 1–9。
2. 每张 prompt 都是完整英文，且包含身份锁硬约束（The reference product has exactly ...）。
3. style_brief 非空（整套统一风格）。
4. TH 站点至少 2 张含泰文原文（逐字文案嵌入，不被翻译成英文）。
5. 无「避免最高级」等自我词语限制（无词语限制原则）。
"""
import sys
import re
...
by_slot = {int(item["slot"]): str(item["prompt"]) for item in prompts if isinstance(item, dict) and item.get("slot")}
assert set(by_slot) == set(range(1, 10)), f"slot 应覆盖 1–9，实际 {sorted(by_slot)}"

# 1. style_brief 非空
assert style_brief, "style_brief 为空"

# 2. 身份锁保真：每张都复述不变量硬约束句
identity_hard_missing = [
    slot for slot, text in by_slot.items()
    if "reference product has exactly" not in text.lower()
]
assert not identity_hard_missing, f"以下槽位缺少身份锁硬约束句：{identity_hard_missing}"

# 2b. 每张都是英文提示词（中文占比应很低，出现的中文只可能是局部说明）
...
# 4. TH 站点：至少 2 张含泰文原文；主图（白底，无文字）不得含文案
thai_hits = [slot for slot, text in by_slot.items() if re.search(r"[฀-๿]", text)]
...
print(f"9 张全部为英文、含身份锁硬约束；slot={sorted(by_slot)}")
print(f"含泰文原文的槽位：{sorted(thai_hits)}（逐字嵌入，未被翻译）")
print("=" * 60)
for slot in range(1, 10):
    text = by_slot[slot]
    print(f"\n--- slot {slot} ({SLOTS[slot - 1]['name']}) 前 180 字 ---")
    print(text[:180])
print("\nPASS")
