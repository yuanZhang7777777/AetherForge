...
3. 不混入原商品参考图，只把用户要修改的对应结果图交给 image2。
4. worker 提交给 image2 的 prompt 只包含用户修改说明、单个标记说明和区域坐标。
"""
import sys
import uuid
...
def main() -> None:
    ...
    try:
        ...
        assert "assets/original-product.png" not in revision.reference_snapshot
        ...
        assert "Edit the provided image according to the user's request." in prompt_text
        assert "整体文字放大，保持商品不变" in prompt_text
        assert "把这里的泰文改清晰" in prompt_text
        assert "x=10%, y=20%, width=30%, height=40%" in prompt_text
        assert "Original ecommerce poster prompt" not in prompt_text

        db.rollback()
        print("PASS: revision generation sends only previous result and user edit prompt")
    finally:
