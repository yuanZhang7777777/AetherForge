...
def n_prepare_single_gpt55_system(site: str) -> str:
    ...
    return (
        ...
        "当用户明确要求某个信息、店铺名称或文案在图片中出现时，必须优先执行，并保持商品事实真实。\n"
        "- 店铺名称是用户提供的店铺/品牌标识素材；它的使用方式、出现位置和视觉表达由商品调性、目标平台和用户要求共同决定。"
        "用户明确要求时必须出现。\n"
        ...
        "# 电商营销美感标准\n"
        "- 每张图都必须有电商设计感，不是普通产品摄影：要有明确视觉焦点、信息层级、商业修图质感和适合手机端的一眼吸引力。\n"
        "- 把粗体营销字体、撞色、发光、速度线、光效、边框、ICON、参数模块、卖点标签、局部放大窗、底栏、对比块、步骤面板当作设计工具库，按品类选择使用。\n"
        "- 强功能/参数商品可使用更强对比、更醒目的标签和信息模块；礼品/饰品/家居/玩具类应使用更符合调性的精致标签、场景道具、材质高光、柔和但有层级的配色。\n"
        ...
        "- 每张 final 必须写清：商品身份、画面目标、构图结构、商品状态、证据信息、图形设计、可见文字、风格质感、商品保持要求。\n"
        ...
        "- 可见文字只写真正需要出现在图片里的消费者可见文案；系统规则、限制条件、缺失信息说明不得进入 VISIBLE TEXT。\n\n"
        "# 缺失参数处理\n"
