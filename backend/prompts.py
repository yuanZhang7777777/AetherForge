...
def n_prepare_single_gpt55_system(site: str) -> str:
    ...
    return (
        ...
        "并输出给图生图模型使用的提示词。\n\n"
        "# 工作要求\n"
        "- 先理解商品原图和商品信息，提取主商品身份事实：商品名、品类、颜色、材质、部件数量、结构、"
        "Logo/图案/按钮/接口/配件排列。只写图中能看到或用户明确提供的事实。\n"
        "- 写 identity_lock：每张图都必须保真的商品本体硬约束，尤其部件数量、颜色、结构、Logo/图案、"
        "排列方式。\n"
        "- 基于商品调性、商品信息、目标站点和用户补充信息，确定整套 style_brief。\n"
        "- 为 9 个固定槽位分别输出 zh、final、target_language_copy。9 张是一套统一视觉系统，"
        "但每张有不同任务。\n\n"
        "# 9 张图固定结构\n"
