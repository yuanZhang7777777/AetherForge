...
def n_prepare_single_gpt55_system(site: str) -> str:
    ...
    return (
        ...
        "- 每张 final 必须写清：商品身份、画面目标、品类适配的电商风格、画面结构、商品状态、真实使用关系、卖点证据、图形信息、字体层级、光线配色和商品保持要求。\n"
        ...
        "- zh 末尾使用“画面可见文字：”列出与 visible_text_lines 完全一致的当地语言短文案，让用户看到的中文执行稿与 final 信息一致。\n"
        ...
        "# 参数与步骤\n"
        "- 精确尺寸、容量、功率、认证和性能数字只使用用户或参考图明确提供的信息；材质、纹理、用途和使用场景可以根据图片、名称和品类常识合理判断。\n"
        "- 没有精确尺寸时，通过手、人物或常见物体建立直观比例，并展示可见材质和工艺，不生成缺参数提示图。\n"
        "- 使用步骤图表现准备、实际操作和完成结果；简单商品不硬凑复杂安装，但各阶段必须出现不同动作和不同商品状态。\n\n"
        "# 真实使用关系\n"
        "- 真实使用、生活方式和步骤画面要表现商品正在发挥用途或完成使用后的结果；人物、手部、内容物、配件和商品之间形成合理关系。\n\n"
        "# 固定九槽\n"
        + _n_prompts_slots_text_adaptive()
        + "\n"
        "# 输出前检查\n"
        "- 九张图围绕参考图中的同一商品，保持其外观、颜色、结构、数量、Logo 和配件。\n"
        "- 每张图完成固定槽位的购买任务，且提示词足以直接支撑一张完整电商图片。\n"
        "- zh、final 与 visible_text_lines 的商品状态、画面内容和当地语言文案相互一致。\n\n"
        + _site_rules(site, local)
        + "\n\n"
        "# 输出格式\n"
...
def _n_prompts_slots_text_adaptive() -> str:
    """APIMart 单节点用：固定 9 图商业任务，槽位内按商品适配。"""
    return (
        "1. 主图：让买家立即看懂商品是什么，并用最重要的购买理由建立第一视觉吸引力。\n"
        "2. 核心卖点图：集中表现一个最有购买价值的核心卖点，通过商品状态、使用结果或视觉证据让买家直接理解。\n"
        "3. 细节特写图：展示最能证明品质或功能的结构、材质、接口、工艺或局部细节。\n"
        "4. 真实使用场景图：展示商品正在被正确使用以及完整使用状态，人物、手部、内容物、配件和商品形成真实用途关系。\n"
        "5. 痛点解决图：选择一个真实消费痛点，清楚展示使用前的问题和使用后的改善。\n"
        "6. 尺寸材质图：帮助买家理解商品大小、结构、材质和工艺；有来源时准确展示尺寸，没有尺寸时用真实比例参照。\n"
        "7. 使用步骤图：展示准备、实际操作和完成结果；简单商品不硬凑复杂安装，各阶段使用不同动作和不同商品状态。\n"
        "8. 生活方式图：展示消费者拥有并使用商品后的生活状态、情绪价值或理想场景，商品真实参与其中。\n"
        "9. 品质信任图：依据商品的材质、结构、做工、包装、配件或检查场景建立购买信任。\n"
    )
...
def retranslate_final_prompt(site: str) -> str:
    """把用户编辑后的完整中文生图提示词忠实重译为英文执行稿。"""
    local = SITES.get(site, {})
    return (
        "你是电商生图提示词翻译器。把用户修改后的完整中文生图提示词 zh 翻译成可以直接交给图像模型的"
        "英文执行稿。只输出 JSON。\n\n"
        + _site_rules(site, local)
        + "\n\n"
        "# 翻译原则\n"
        "- 用户编辑后的 zh 是本张画面的唯一创意来源；完整保留其中的商品状态、真实使用关系、场景、动作、"
        "构图、道具、图形信息、排版、光线、配色和商业质感，不自行改写创意方向。\n"
        "- 身份锁和商品补充信息只用于保持同一商品及明确事实，不替用户重新设计画面。\n"
        "- zh 的“画面可见文字”段以及兼容字段中的当地文案都属于最终画面内容；当地语言字符原样保留，"
        "在英文稿中说明其字体层级、位置、对比度和版式关系。\n"
        "- final 写成一份连贯、完整、可直接执行的英文电商生图提示词，不输出审核说明、事实缺失说明或系统规则。\n"
        "- 只输出 {\"final\": \"完整英文生图提示词\"}。"
    )
...
def retranslate_final_user(
    ...
    lines = [
        f"站点：{site}（消费者可见文案语言：{SITES.get(site, {}).get('copy', site)}）",
        f"商品身份参考：{identity_lock}",
        f"商品补充信息：{'；'.join(points) if points else '(未提供)'}",
        f"用户编辑后的完整中文生图提示词（以此为准）：\n{zh}",
        f"兼容保存的当地语言文案（与中文稿中的画面可见文字一致）：\n"
        f"{target_language_copy or '(无)'}",
        "",
        '只输出一个 JSON 对象 {"final": "完整英文生图提示词"}。',
    ]
    return "\n".join(lines)
