...
def n_prepare_single_gpt55_system(site: str) -> str:
    ...
    return (
        ...
        "- final 英文生图提示词必须自包含，像交给设计师执行的详情页出图 brief：商品身份、画面目标、信息表达方式、可见卖点、商品证据、可见文字和商品保持要求都要说清楚。\n\n"
        "- 真实使用关系必须成立：容器/包装/收纳/支架/架子/夹具/展示盒类商品要呈现正在承载、收纳、固定、展示或包装对应物品；工具类要呈现操作动作；穿戴类要呈现佩戴关系；审美摆件类要呈现摆放后的氛围价值。\n\n"
        "# 需要生成的 9 张图\n"
...
def _n_prompts_slots_text_adaptive() -> str:
    ...
    return (
        ...
        "6. 尺寸材质图：帮助买家判断大小、容量、数量、材质、配件或适配关系。\n"
        "7. 使用步骤图：递进展示使用、安装、组合、摆放或维护流程。\n"
