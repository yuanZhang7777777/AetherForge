...
def n_prepare_single_gpt55_system(site: str) -> str:
    ...
    return (
        ...
        "# 输入信息\n"
        "- 商品原图、用户填写的商品名称、店铺名称、补充信息、图片中可见文字都只是参考信息，请自行理解后用于设计。\n"
        "- 如果用户填写了店铺名称，把它当作可用的店铺/品牌标识素材；不要默认忽略，也不要固定位置，由你判断是否适合出现在某些图片中以及如何融入版式。\n\n"
        "# 转化信息层级\n"
        ...
        "- 每张图的 zh 中文策划必须写清，并像一张可直接执行的电商详情页设计稿，按顺序写清：买家疑问、信息模块、商品证据、画面层级、可见文字、事实限制。\n"
        "- 买家疑问：这张图要回答消费者下单前的哪个疑问，例如是什么、适合谁、怎么用、好在哪里、细节是否可信、尺寸是否合适、收到什么、是否值得买。\n"
        "- 信息模块：选择最适合该疑问的表达模块，可用但不限于卖点解释、局部放大、引线标注、前后对比、场景演示、步骤拆解、尺寸材质、配件清单、品质信任；模块由商品和事实决定，不固定套用。\n"
        "- 商品证据：明确写出画面里用哪些可见商品结构、材质、配件、包装、使用动作、场景道具或用户补充信息来证明卖点；没有证据的内容不要写。\n"
        "- 画面层级：说明主视觉、标题区、卖点区、标注区、对比区、辅助道具和留白如何安排，让手机端第一眼先看到商品和核心信息，再看到证据细节。\n"
        "- 可见文字：写出适合目标站点语言的短标题、短卖点或标注文字；文字要像电商详情页里的销售说明，不要空泛品牌口号。\n"
        "- final 英文生图提示词必须自包含，像交给设计师执行的详情页出图 brief：商品身份、信息模块、构图、画面层级、文字区域、可见卖点、商品证据和保真限制都要说清楚。\n\n"
        "# 需要生成的 9 张图\n"
